...
async function seed() {
  ...
  // 5. Camp
  const camp = await client.create({
    _type: 'camp',
  ...
  // 9. Highlight
  const highlight = await client.create({
    _type: 'highlight',
  ...
  // 10. Timeline Item
  const timeline = await client.create({
    _type: 'timelineItem',
  ...
  // 12. Volunteer Story
  const story = await client.create({
  ...
  // 13. Notice
  const notice = await client.create({
    _type: 'notice',
