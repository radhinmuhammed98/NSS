...
import { HighlightCard, SafeImage } from "@/components/media";
  ...
  notFoundComponent: () => (
    <PageShell>
      <Container className="py-20 text-center">
        ...
        <Link to="/camps" className="mt-4 inline-block text-primary">
          ← Back to camps
        </Link>
      </Container>
    </PageShell>
    ...
    <PageShell>
      ...
      <Container className="py-8">
        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
          ...
          <span className="flex items-center gap-1">
            <Calendar className="h-4 w-4" /> {formatDate(camp.startDate)} –{" "}
            {formatDate(camp.endDate)}
          </span>
          <span className="flex items-center gap-1">
            <Users className="h-4 w-4" /> {camp.volunteerCount} volunteers
          </span>
        ...
        <ClayCard tilt={false} className="mt-6">
          ...
          <p className="mt-3 text-sm">
            <span className="font-semibold">Programme Officer:</span> {camp.programmeOfficer}
          </p>
          <div className="mt-2 flex flex-wrap gap-2">
            {camp.campLeaders.map((l: string) => (
              <Badge key={l}>{l}</Badge>
            ))}
          </div>
        </ClayCard>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3">
          {camp.impactMetrics.map((m: ImpactMetric) => (
                    ...
                    <div className="mt-3 flex gap-3">
                      {d.images.map((im: ImageAsset) => (
                        <SafeImage
                          ...
                          loading="lazy"
                          decoding="async"
                          className="clay-sm h-20 w-28 object-cover p-0"
                        />
                      ))}
                    </div>
                  )}
