...
function Contact() {
  const { s } = Route.useLoaderData() as { s: any };
  usePageMeta({
    title: "Contact",
    description: `Get in touch with NSS Unit 466 at ${s.schoolName || "KHMHSS Valakkulam"}. Find our address, email, and phone number.`,
  });
            ...
            <ClayCard tilt={false} className="nss-flex nss-flex-col nss-p-4 nss-sm-p-6" style={{ height: "100%" }}>
              ...
              <ul className="nss-mt-5 nss-flex nss-flex-col nss-gap-4 nss-text-sm" style={{ listStyle: "none" }}>
                <li className="nss-flex nss-items-start nss-gap-3">
                  <MapPin className="nss-shrink-0" style={{ height: "1.25rem", width: "1.25rem", color: "var(--primary)", marginTop: "2px" }} aria-hidden="true" /> 
                  <div>
                ...
                <li className="nss-flex nss-items-start nss-gap-3">
                  <Mail className="nss-shrink-0" style={{ height: "1.25rem", width: "1.25rem", color: "var(--primary)", marginTop: "2px" }} aria-hidden="true" /> 
                  <div>
                ...
                {s.phone && (
                  <li className="nss-flex nss-items-start nss-gap-3">
                    <Phone className="nss-shrink-0" style={{ height: "1.25rem", width: "1.25rem", color: "var(--primary)", marginTop: "2px" }} aria-hidden="true" /> 
                    <div>
          ...
          <Reveal delay={0.1}>
            <ClayCard tilt={false} className="nss-flex nss-flex-col nss-p-4 nss-sm-p-6" style={{ height: "100%" }}>
              ...
              <div className="nss-mt-5" style={{ paddingTop: "1rem", borderTop: "1px solid var(--border)", opacity: 0.8, fontSize: "14px", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                <p><span className="nss-font-semibold">Principal:</span> {s.principal || "Asif PA"}</p>
                <p><span className="nss-font-semibold">Programme Officer:</span> {s.programmeOfficer || "Dr. Broose KV"}</p>
              </div>
